from ..models import User, Agent, Localisation
...
users_router = Blueprint("users_router", "users_router", url_prefix="/api/v1/user")
...
def get_all_users():
    try:
        ...
        return Response({"code": 200, "users": documents}), 200
...
def save_agent():
    try:
        ...
        return Response({"code": 200, "data": agentData})
    except pymongo.errors.PyMongoError as e:
        return jsonify({"error": "user not created", "details": str(e)}), 400
    except Exception as e:
        return jsonify({"error": "user not created", "details": str(e)}), 400
    except TypeError as error:
        return jsonify({"error": "user not created", "details": str(error)}), 400
...
def user_login():
    try:
        ...
        if users is None or len(users) == 0:
            return {"code": 404, "message": f"{loginStr} ou mot de passe incorrect"}
        ...
        agent = agent[0]
        pointing = Localisation().find({"agent": agent.get("_id")})

        agent["as_user"] = user
        agent["pointing"] = pointing
        ...
        return jsonify({"error": "échec de connexion", "details": str(e)}), 400
